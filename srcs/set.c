...
/*   Updated: 2024/08/21 17:31:15 by tigpetro         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include <minishell.h>

static t_set_node	*__make_set_node__(t_node *node)
{
	t_set_node	*set_node;
...
void	ft_insert_set(t_set *set, t_node *node)
{
	...
	if (ft_empty_set(set))
		set->head = __make_set_node__(node);
	else
	{
		curr = set->head;
		while (curr->next)
			curr = curr->next;
		curr->next = __make_set_node__(node);
	}
}
